...
screen messenger():
    ...
    frame background None xysize (600,975) at move_messenger:
                                ...
                                hbox spacing -250 xalign message.position-xoffset:
                                    hbox spacing -50:
                                        button xysize 343,87 background audio_idle hover_background audio_hover action If (now_play, [Stop('sound')], [SetScreenVariable('played', True), SetScreenVariable('audio_sec', 0), Function(message.audio.play)]):
                                            text "{k=1.5}%s{/k}"%(message.audio.duration) style 'txt_base' align .26,.9 size 22 color style_gray
                                        if message_time:
        ...
        text "%s"%(interlocutor) style 'txt_base' size 32 xalign 0.25 yalign 0.025
        add status align 0.25, 0.065
        ...
        if find_enable:
            input changed find_change_func length 14 align .12,.99 color style_green
