label javier_1:
    play music friendcore if_changed volume 1.5
    scene couch
    show parvez happy at midleft
    with fade
    cg "so that dude was able to propagate the plants???"
    cg "from your jizzz???"
    pdm "hahaha"
    pdm "from his jizz too"
    cg "can i have one??"
    cg "wait is that weird to ask lmao????!"
    pdm "for my jizz lol"
    pdm "sure you can have that"
    pdm "(jk ... or not haha)"
    pdm "..."
    pdm "fr tho you can have a plant if you want"
    pdm "terry is really good at propagating stuff"
    pdm "we already have like six"
    cg "holy shit dude"
    cg "that's amazing"
    cg "you could start a nursery of them"
    cg "a whole garden of dickplants"
    cg "um ok so ... inanna is at her mom for the weekend"
    pdm "at your mom you mean lmao"
    cg "shut up"
    cg "so like ... if you wanna come over ..."
    scene inannabed
    show parvez happy:
        xalign 0.07
    show javier happy at midleft
    with wipeleft
    "The vibe of Inanna's place--Parvez realizes that's how he thinks of it, as Inanna's place--is incredibly different without her."
    "For one thing when Javier opens the door a fog of weed smoke issues forth."
    "The place is kind of a mess, but xe is still wearing crisp spikey eyeliner and a cute blush on their nose."
    "Xe grins from ear to ear at the sight of Parvez, almost crushing the plant when xe throws xer arms around him."
    p "waugh!"
    show javier worried with dissolve
    j "Oh jeez .. sorry haha"
    show javier happy with dissolve
    "Xe is so much bubblier like this (stoned out of their mind)."
    "Xe offers Parvez a hit from the volcano, and soon his head is spinning too."
    "It feels like they're two kids having a sleepover, sitting together on mom's bed."
    if inanna_sent_me == True:
        "It feels extra transgressive to Parvez because not only did she tell him not to fuck anyone, he's going to fuck her partner."
        "... Parvez supposes he doesn't {i}know{/i} he's gonna fuck Javier ..."
        "... but he has a feeling ..."
    "Javier pulls out a basket full of tarot decks and offers to read Parvez's fortune.{w} For some reason this involves touching his leg a lot."
    j "The hanged one is like ...{w} Well, it means you were in a transitional period."
    j "You've been made to wait, reflect a while. You weren't able to move forward on your path, and you had to wait a while, and kinda reflect, gestate new ideas."
    p "Like, a pregnancy?"
    j "Yeah!"
    j "That's your past card, and the present card is the seven of wands."
    j "That's like, you've got all this fire and creativity that you've grown inside you, and now you're spreading it to others."
    p "I definitely have been spreading my wand around ... haha"
    "Javier blushes."
    j "Yeah that too I guess."
    p "And the future?"
    j "The page of wands. You're like a knight for passion and energy and creativity!!"
    "Xe giggles and it makes Parvez laugh too."
    "Nothing xe said really makes sense to him, probably because of all the weed."
    show javier happy at furthestright
    with move
    "Javier suddenly gets up from the bed."
    j "Wanna see something cool?"
    "Xe strips naked."
    "Parvez, totally shocked, starts laughing."
    show javier worried with dissolve
    j "What??"
    p "Dude what yourself oh my god"
    "He's still cackling."
    p "You just took all your clothes off."
    "Parvez gathers himself, seeing that Javier looks a bit embarrassed now."
    p "Sorry, I just ... I used to think you were really intimidating."
    p "Now that I know you, you're kind of a dork.{w} A really cute one, though."
    show javier happy with dissolve
    j "I just wanted to show you this--"
    scene cg J1A with dissolve
    play music maintheme
    pause
    "They sit up on the bedside table and spread their labia."
    "Their pussy looks delicious, hair neatly trimmed, tight and squeezable butt."
    j "Look, isn't it weird?"
    "Xe spreads xemself open further. It's so pink inside."
    p "What's weird?"
    j "No dick!!"
    "Oh yeah. Just a regular pussy."
    p "Did the seeds not work? I thought you said you grew one too."
    j "I did but, it's kinda different than yours ...."
    "Xe starts rubbing xemself, tugging on xer clit, petting xer labia, and then sliding xer fingers inside."
    "Parvez stares openly, unselfconscious, fixated on the show."
    "Xer fingers are making slick sounds inside xemself."
    "Xe spreads xer hole open with two fingers."
    j "Are you looking??"
    "Fuck, how could Parvez not?"
    j "Do you see it?"
    "Parvez sees xem digging inside xer cunt, with a mixture of arousal and concentration on xer face."
    "Aside from that nothing seems out of the ordinary, and then he sees it."
    "When xe spreads xer fingers again, something green is in there."
    "Xe keeps stroking inside xemself, like xer coaxing something out."
    j "There they are!"
    "Sure enough, xer cock emerges slowly from inside xer pussy, first little, then getting bigger."
    scene cg J1B with pushright
    pause
    "Javier's cock is thinner than Parvez's, long and bright green and a little pointy."
    "Xe grins at him brightly."
    j "Isn't that weird !!"
    "Parvez gets up and goes to xem, wanting immediately to touch it."
    "Similarly to his own cock, it's not quite warm to the touch the way human flesh is, a bit cool."
    "Javier reacts to the touch, whining slightly."
    j "Ah, you're squeezing so tightly."
    p "Sorry ... you're just really hot ..."
    j "Hehe ... it's okay ..."
    "Parvez strokes them off for a few moments before realizing he's hard too."
    "He pulls his pants down and slots his dick in alongside Javier's, rubbing them both with one hand."
    "Javier whines and arches up against him."
    "Parvez can feel the slickness from their pussy rubbing up against him."
    p "Hey can I still fuck you?"
    j "Y-yeah."
    "Parvez presses his cock against Javier's slit, right below the place where Javier's own shaft emerges."
    scene cg J1C with dissolve
    pause
    "Javier pushes down against him, levering xemself off the bedside table with xer arms."
    "They bounce on his cock, surprisingly strong."
    "Xer pussy is incredibly tight, squeezing Parvez almost to the point of pain."
    "All he wants is to keep pushing in though."
    "He presses Javier back onto the table surface and fucks into xem, xer cock pressing against his belly."
    j "Ah--ahh--"
    p "Is it too hard?"
    j "No, harder."
    "Parvez rams them full force, as much and as hard as he can. Xe wails and claws at his back."
    "He can't cum and neither can xe, both of them between the times of the month when seeds come out."
    "Parvez just keeps pounding xem, kissing them forcefully, rubbing their tongues together inside Javier's mouth the same way their cocks are rubbing together inside xer pussy."
    "They fuck hard, and then slow, and then hard again."
    "Parvez' legs go numb and they swap places, Javier riding him on the bed, then fucking him against the table, then back to their original position."
    "Eventually they get too sweaty, and worn out, legs twitching and asses cramped."
    scene inannabed
    show parvez happy:
        xalign 0.07
    show javier happy at midleft
    with dissolve
    "They collapse together on the bed and smoke more weed, languidly making out, and then stroking each other again."
    "At some point they fall asleep. Parvez wakes up in the middle of the night with a tarot card stuck to his cheek--the star."
    "The illustration is of a woman in a bra and a labcoat, with a pot of clear liquid on a hot plate in front of her."
    "With one hand she pours liquid from a beaker into the pot, and with the other she squeezes it through a syringe into a little vial."
    "Her thumb is over the rim of the beaker, her finger just in the viscous liquid."
    "It doesn't seem sanitary to Parvez to cook hormones seminude and bare-handed, but it seems somehow right."
    "The woman is haloed by eight stars--seven small ones and one large one--which form the colors of the trans flag around her."
    "He wishes he could ask Javier what the card means, but xe is soundly sleeping."
    "Instead he sticks it to xer forehead as they snore, sprawled out naked on top of the sheets, and heads home."
    $ javier_1_complete = True
    if inanna_sent_me == False:
        scene cardboard with fade
        call screen chapterselect
    else:
        $ obedience_points -= 5
        "He's going to get in so much trouble with Inanna ..."
        "Javier would for sure tell her that they fucked ..."
        $ inanna_sent_me = False
        jump obedience_check_3